...
export default class View {
  ...
  listStruct(newStruct) {
    ...
    if (newStruct instanceof Line) {
      row.id = "L-" + newStruct.id
      T.addCell(row, "/", "line")
    }
    if (newStruct instanceof Circle) {
      row.id = "C-" + newStruct.id
  ...
  hover(evt) {
    ...
    if (this.hasClass("Point")) {
      ...
      expression = `[ ${A.kat(element.x)}, ${A.kat(element.y)} ]`
    }

    if (this.hasClass("Circle")) {
      id = this.attr('struct-id');
      listId = "#C-" + id
      element = window.M.structs[id];
      // expression = katex.renderToString(element.eq.getSimple())
      // latex = "0 = " + A.run(`printlatex(${element.eq.simple0})`)
      latex = A.run(`printlatex(${element.eq.simple0})`) + " = 0"
      console.log(latex);
      expression = katex.renderToString(latex)
    ...
    if (this.hasClass("Line")) {
      ...
      element = window.M.structs[id];
      // expression = katex.renderToString(element.eq.getSimple())
      // expression = katex.renderToString(element.eq.getSimple0())
      // latex = "0 = " + A.run(`printlatex(${element.eq.simple0})`);
      latex = A.run(`printlatex(${element.eq.simple0})`) + " = 0"
      console.log(latex);
      expression = katex.renderToString(latex)
    ...
    if (this.hasClass("Segment")) {
      ...
      expression = katex.renderToString(element.length)
    }

    // let info = element.type + " " + element.id + ":  " + katex.renderToString(expression);
    let info = element.type + " " + element.id + ":  " + expression
    // console.log(info);
    window.V.footerPanel.innerHTML = info

    let row = document.querySelector(listId)
    if (row) {
      // row.scrollIntoView();
